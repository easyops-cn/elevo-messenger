...
function handleDeepLink(rawUrl: string, navigate: ReturnType<typeof useNavigate>) {
  ...
  if (parsed.pathname.startsWith(SSO_CALLBACK_PATH)) {
    ...
    const routePath = `${parsed.pathname.slice(SSO_CALLBACK_PATH.length)}?loginToken=${encodeURIComponent(loginToken)}`;
    navigate(routePath, { replace: true });
    return;
  }

  if (parsed.pathname.startsWith(OIDC_CALLBACK_PATH)) {
